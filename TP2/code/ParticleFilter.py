...
def motion_model(x, u_tilda, dt_pred, QEst):
    ...
    w = np.random.multivariate_normal([0, 0, 0], QEst).reshape(3, 1)
...
def resampling_systematic(px, pw):
    """
...
def resampling_stratified(px, pw):
    """
    Stratified Resampling
    """
    w_cum = np.cumsum(pw)
    re_sample_id = (np.arange(nParticles) + np.random.uniform(0, 1, nParticles)) / nParticles
    indexes = []
    ind = 0
    for u in re_sample_id:
        while u > w_cum[ind]:
            ind += 1
        indexes.append(ind)
    px = px[:, indexes]
    pw = np.ones(pw.shape) / nParticles

    return px, pw


def resampling_multinomial(px, pw):

    """
    Multinomial Resampling
    """
        
    indexes = np.random.choice(range(nParticles), size=nParticles, p=pw)
    px = px[:, indexes]
    pw = np.ones(pw.shape) / nParticles
    return px, pw
...
#    print(k)
    ...
    if z is not None:
        for p in range(nParticles):
            ...
            expo = -0.5 * (Innov.T @ np.linalg.inv(REst) @ Innov)  # (1x1)
            wp[p] = wp[p] * np.exp(expo[0, 0])
    # Normalization
    wp = wp / np.sum(wp) 
    ...
    PEst = (wp * (xParticles - xEst)) @ (xParticles - xEst).T #
    xSTD = np.sqrt(np.diag(PEst)).reshape(3, 1) #
    
    resampling_method = "stratified"  # or "systematic", "multinomial"
    ...
    if Neff < Nth:
        ...
        if resampling_method == "systematic":
            xParticles, wp = resampling_systematic(xParticles, wp)
        elif resampling_method == "stratified":
            xParticles, wp = resampling_stratified(xParticles, wp)
        else:
            xParticles, wp = resampling_multinomial(xParticles, wp)
...
# fig, ax = plt.subplots()
# plt.title("theta_eff: 1.0")
# ax.hist(hwp)
# fig.savefig("hist 1.0"+".png")
